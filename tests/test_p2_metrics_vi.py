...
class TestP2MetricsVi(unittest.TestCase):
    def test_labels_and_ids(self):
        ...
        self.assertIn("Deny có gọi LLM", html)
        self.assertIn("Cổng (tháng)", html)
        self.assertIn("Hard Deny", html)
        self.assertIn("Tổng chat", html)
        self.assertIn("Tổng deny", html)
        self.assertIn("Hard Deny không gọi LLM · Deny có gọi LLM phải = 0", html)
